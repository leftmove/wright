...
use typst_library::html::{attr, tag, FrameElem, HtmlDocument, HtmlElem, HtmlElement, HtmlNode};
use typst_library::introspection::{Introspector, Locator, LocatorLink, SplitLocator, TagElem};
...
fn handle(
    ...
    if let Some(elem) = child.to_packed::<TagElem>() {
        ...
        let children = html_fragment(engine, &elem.body, locator.next(&elem.span()), styles)?;
        output.push(
...
fn root_element(output: Vec<HtmlNode>, info: &DocumentInfo) -> SourceResult<HtmlElement> {
    ...
    Ok(HtmlElement::new(tag::html).with_children(vec![head_element(info).into(), body.into()]))
}
...
fn classify_output(mut output: Vec<HtmlNode>) -> SourceResult<OutputKind> {
    ...
    for node in &mut output {
        let HtmlNode::Element(elem) = node else {
            continue;
        };
        let tag = elem.tag;
