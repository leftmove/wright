...
use typst_library::visualize::{ExchangeFormat, FillRule, Image, Paint, RasterImage, RelativeTo};
...
impl SVGRenderer {
    ...
    pub(super) fn render_text_group(&mut self, state: State, text_group: &[(Point, &TextItem)]) {
        // The `state` transform has the position of the first text item.
        let first_pos = text_group[0].0;
        ...
        for (pos, text) in text_group {
            let relative_pos = *pos - first_pos;
            let mut x = relative_pos.x.to_pt();
            let scale: f64 = text.size.to_pt() / text.font.units_per_em();
            ...
            for glyph in &text.glyphs {
                ...
                // Render glyphs (this part needs refactoring from render_text)
                self.render_svg_glyph(text, id, offset, scale)
                    .or_else(|| self.render_bitmap_glyph(text, id, offset))
                    .or_else(|| {
                        self.render_outline_glyph(
        ...
        self.xml.end_element(); // g for visuals

        // Selectable text overlay
        self.xml.start_element("text");

        let first_text = text_group[0].1;
        self.xml.write_attribute("x", &0.0);
        self.xml.write_attribute("y", &0);

        self.xml.write_attribute("fill", "transparent");
        self.xml.write_attribute("stroke", "none");
        self.xml
            .write_attribute("text-rendering", "geometricPrecision");
        let font_info = first_text.font.info();
        self.xml.write_attribute("font-family", &font_info.family);
        self.xml
            .write_attribute_fmt("font-size", format_args!("{}pt", first_text.size.to_pt()));
        ...
        for (pos, text) in text_group {
            ...
            self.xml.write_attribute("x", &relative_pos.x.to_pt());
            if text.font.info() != first_text.font.info() {
                let font_info = text.font.info();
                self.xml.write_attribute("font-family", &font_info.family);
            }
            if text.size != first_text.size {
                self.xml
                    .write_attribute_fmt("font-size", format_args!("{}pt", text.size.to_pt()));
            }
            self.xml.write_attribute("textLength", &width);
            self.xml.write_attribute("lengthAdjust", "spacing");

            self.xml.write_text(text.text.as_str());
            self.xml.end_element(); // tspan
        }

        self.xml.end_element(); // text
    }

    /// Render only the selectable text overlay for a group of text items.
    pub(super) fn render_text_group_overlay_only(
        &mut self,
        _state: State,
        text_group: &[(Point, &TextItem)],
    ) {
        let first_pos = text_group[0].0;
