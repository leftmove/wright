...
use typst_library::layout::{
    Abs, Frame, FrameItem, FrameKind, GroupItem, Page, PagedDocument, Point, Ratio, Size, Transform,
};
...
/// Export a frame into a SVG file.
#[typst_macros::time(name = "svg selectable")]
pub fn svg_selectable(page: &Page) -> String {
    let mut renderer = SVGRenderer::new();
    renderer.write_header(page.frame.size());

    let state = State::new(page.frame.size(), Transform::identity());
    renderer.render_page_selectable(state, Transform::identity(), page);
    renderer.finalize()
}

/// Export only the selectable text overlay for a page into an SVG file.
#[typst_macros::time(name = "svg text overlay")]
pub fn svg_text_overlay(page: &Page) -> String {
    let mut renderer = SVGRenderer::new();
    renderer.write_header(page.frame.size());

    let state = State::new(page.frame.size(), Transform::identity());
    renderer.render_page_text_overlay(state, Transform::identity(), page);
    renderer.finalize()
}

impl SVGRenderer {
    fn render_page_text_overlay(&mut self, state: State, ts: Transform, page: &Page) {
        // Do not draw shapes/images; only recurse into frame for text overlay.
        self.render_frame_text_overlay(state, ts, &page.frame);
    }

    fn render_frame_text_overlay(&mut self, state: State, ts: Transform, frame: &Frame) {
        ...
        let mut i = 0;
        while i < items.len() {
            let (pos, item) = items[i];

            match item {
                FrameItem::Text(text) => {
                    let mut text_items = vec![(*pos, text)];
                    let mut j = i + 1;
                    let mut current_pos = *pos;
                    while j < items.len() {
                        let (next_pos, next_item) = items[j];
                        if let FrameItem::Text(next_text) = next_item {
                            if (next_pos.y - current_pos.y).abs() < Abs::pt(1.0) {
                                text_items.push((*next_pos, next_text));
                    ...
                    self.render_text_group_overlay_only(state.pre_translate(*pos), &text_items);
                    self.xml.end_element();
                    i = j;
                }
                FrameItem::Group(group) => {
                    // Recurse into group, preserving transform/size semantics.
                    let next_state = match group.frame.kind() {
                    ...
                    self.xml
                        .write_attribute_fmt("transform", format_args!("translate({x} {y})"));
                    self.render_frame_text_overlay(next_state, group.transform, &group.frame);
                    self.xml.end_element();
                    i += 1;
                }
                _ => {
                    // Ignore non-text items for overlay.
                    i += 1;
                }
            }
        }
...
impl SVGRenderer {
    ...
    fn render_page_selectable(&mut self, state: State, ts: Transform, page: &Page) {
        if let Some(fill) = page.fill_or_white() {
            let shape = Geometry::Rect(page.frame.size()).filled(fill);
            self.render_shape(state, &shape);
        }

        self.render_frame_selectable(state, ts, &page.frame);
    }

    fn render_frame_selectable(&mut self, state: State, ts: Transform, frame: &Frame) {
        self.xml.start_element("g");
        if !ts.is_identity() {
            self.xml.write_attribute("transform", &SvgMatrix(ts));
        }
        ...
        while i < items.len() {
            ...
            match item {
                FrameItem::Text(text) => {
                    ...
                    self.xml
                        .write_attribute_fmt("transform", format_args!("translate({x} {y})"));
                    self.render_text_group(state.pre_translate(*pos), &text_items);
                    self.xml.end_element();
    ...
    fn render_group(&mut self, state: State, group: &GroupItem) {
        ...
        if let Some(clip_curve) = &group.clip {
            ...
            let id = self
                .clip_paths
                .insert_with(hash, || shape::convert_curve(clip_curve));
            self.xml
